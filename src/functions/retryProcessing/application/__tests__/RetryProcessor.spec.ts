...
const checkProcessSuccessfulUpdatedTestResult = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM TEST_RESULT WHERE application_reference = 1
...
const checkErrorsToRetryUpdatedUpLoadQueues = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT COUNT(*) as rowcount FROM UPLOAD_QUEUE WHERE application_reference = 2
...
const checkErrorsToAbortUpdatedTestResult = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT *  FROM TEST_RESULT WHERE application_reference = 3
...
const checkSupportInterventionUpdatedUploadQueues = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT COUNT(*) as rowcount FROM UPLOAD_QUEUE WHERE application_reference = 4
    and staff_number = '1234' and upload_status = 0;`,
           [], (err, row) => {
             if (err || !row) {
               reject('Row not found or incorrect state');
...
const checkSupportInterventionUpdatedTestResult = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM TEST_RESULT WHERE application_reference = 4
...
const checkOldEntryCleanupDeleteUploadQueues = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT COUNT(*) as rowcount FROM UPLOAD_QUEUE WHERE application_reference = 5
    and staff_number = '1234';`,
           [], (err, row) => {
             if (err || !row) {
               reject('Row not found or incorrect state');
