  ...
  return new Promise((resolve, reject) => {
    db.query(
      ...
      (err, results, fields) => {
        ...
        resolve((results as mysql.RowDataPacket[]).map(
          (row) => ({ application_reference: row.application_reference, interface: row.interface }))
        );
      });
  });
};
