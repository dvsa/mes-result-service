...
export const duplicateQuery = (): string => {
  const template =
    `INSERT INTO DUPLICATE_CERTIFICATES (pass_certificate_number, times_cert_used, test_details)
        ...
        HAVING times_cert_used > 1
        ) # Identify duplicate certificates
  ON DUPLICATE KEY UPDATE times_cert_used=VALUES(times_cert_used),test_details=VALUES(test_details)`;
  const args = [];
